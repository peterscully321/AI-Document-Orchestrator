...
├── __init__.py
├── models.py              # Data models (UserInput, RawData, StructuredNotes, etc.)
├── base_agent.py          # Base agent class with A2A handoff support
├── agents/
...
├── tools/
│   ├── search.py          # Tavily/Google Search tool
│   ├── mcp_filesystem.py  # MCP File System Read tool
│   ├── google_docs.py     # Google Docs API tool
│   └── pdf_generator.py   # PDF generator tool
└── orchestrator/
    └── orchestrator.py    # Main orchestrator coordinating all phases
```
...
**Quick summary:**
- ✅ **Required:** `GOOGLE_GEMINI_API_KEY` - Get from https://makersuite.google.com/app/apikey
- ✅ **Required:** `TAVILY_API_KEY` - Get from https://tavily.com
- ⚪ **Optional:** `GOOGLE_CREDENTIALS_PATH` - Only needed for Google Docs output
...
The UI includes:
- 📝 **Document Generator** - Full document generation with research
- 📄 **Summarizer** - Quick content summarization
- ✍️ **Blog Writer** - Create engaging blog posts
- ⚙️ **Settings** - API configuration and system info
